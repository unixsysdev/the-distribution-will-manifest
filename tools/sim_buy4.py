...
mint_pk = Pubkey.from_string(MINT); fee_recip = Pubkey.from_string("CebN5WGQ4jvEPvsVU4EoHEpgzq1VV7AbicfhtW4xC9iM")
...
async def main():
    async with AsyncClient(config.rpc_http_url()) as cli:
        ...
        cu = [set_compute_unit_limit(200_000), set_compute_unit_price(2_000_000)]  # priority fee
        ata = build_ata_create_idempotent_ix(user, user, mint_pk, tp)
        buy = build_buy_ix(mint_pk, user, fee_recip, et, mc, token_program=tp, creator=creator)
        bh = (await cli.get_latest_blockhash()).value.blockhash
        msg = MessageV0.try_compile(payer=user, instructions=cu + [ata, buy], address_lookup_table_accounts=[], recent_blockhash=bh)
        sim = await cli.simulate_transaction(VersionedTransaction(msg, [wallet]))
        print("buy+priority-fee SIM ERR:", sim.value.err)
        print("last log:", (sim.value.logs or ["(none)"])[-1])
asyncio.run(main())
