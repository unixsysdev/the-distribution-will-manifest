"""continuation_live_rep_bot.py — LIVE continuation bot, LEAN+REP methodology (2026-06-15).

The validated edge: filter each 2x cross with the rich 22-feature tracker + the buyer-cluster
REPUTATION (which recurring bots are piling in). Driven by the direct gRPC listener + the intent
SHM ring; scores with the trained LEAN+REP model; the reputation map is seeded from 6 days of
history and kept current as positions resolve.
...
from .continuation_bot import ContinuationBot, parse_args, _empty, PUMP_RT, NARROW, LAMPORTS_PER_SOL, STATUS
from .continuation_tracker_rich import RichTracker
from .continuation_reputation import Reputation
from .continuation_sizing import plan_buy, simulate_fill, realized_return

ROOT = "/root/the-distribution-will-manifest"
MODEL = f"{ROOT}/bot_data/cont_lean_model.pkl"      # clean LEAN; rep dropped 2026-06-15 (dead-rep serve skew, no cross-period edge)
SPEC = f"{ROOT}/bot_data/cont_lean_features.json"
SEED = f"{ROOT}/bot_data/cont_reputation_seed.json"
...
REP_LOG = f"{ROOT}/bot_data/continuation_rep.jsonl"
TIMING_LOG = f"{ROOT}/bot_data/cont_timing.jsonl"   # per-cross hot-path stage latencies (instrumentation, 2026-06-16)
POS_JOURNAL = f"{ROOT}/bot_data/cont_open_positions.json"  # LIVE open positions, persisted every mutation for restart recovery (2026-06-16)
IGNORE_MINTS_FILE = f"{ROOT}/cont_ignore_mints.txt"        # mints the bot never recovers/adopts/manages (old stranded bags); one mint per line, '#' comments
...
class RepLive(ContinuationBot):
    ...
    async def run(self):
        ...
        print(f"[rep-bot] gRPC listener starting (live={self.live}, LEAN clean, rep seed {self.rep.stats()})", flush=True)
        await grpc_listener_for_harness(self)
