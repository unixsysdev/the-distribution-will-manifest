...
Reads bot_data/continuation_status.json. Refresh 1s, read-only.
Run from the repository root: `python -m research.continuation.continuation_dashboard`.
"""
...
class ContinuationDashboard(App):
    ...
    def _render(self):
        s = load(self.root / "bot_data" / "continuation_status.json")
        if not s:
            self.query_one("#hdr", Static).update(Text("waiting for continuation_bot status…", style="yellow")); return
        age = time.time() - s.get("ts", 0); mode = s.get("mode", "?")
        h = Text()
        h.append("CONTINUATION BOT ", style="bold white")
